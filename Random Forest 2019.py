...
for month in total_consumption_2019:
    print(f"Simulating for {month}:")
    days = np.arange(1, len(total_consumption_2019[month]) + 1)
    
    # Training model
    model = train_consumption_model(total_consumption_2019[month], 
    ...
    for i, real_consumption in enumerate(total_consumption_2019[month]):
...
plt.plot(real_consumptions, label='Real Consumption (2019)', color='blue', marker='.')
plt.plot(simulated_consumptions, label='RandomForestRegression Simulated Consumption', color='red', linestyle='--', marker='.')
plt.title('Comparison of Real vs RandomForestRegression Simulated Electricity Consumption (2019)')
plt.xlabel('Days')
